...
class BaseEmrWorkflow():
    ...
    def _fetch_hive_interim_dir(params):
        source_app = params.get('source_app', 'milkyway')
        return Variable.get(f"{source_app}_hive_interim_dir")

    @staticmethod
    def _fetch_hive_cli_dir():
    ...
    def _get_dates(self, params):
        ...
        while (snapshot_start <= snapshot_end):
            ...
            if new > snapshot_end:
                if days != 1:
                    result.append((
                        self._datetime_format(snapshot_start),
                        self._datetime_format(snapshot_end)
                    ))
                break
            else:
                result.append((
                    self._datetime_format(snapshot_start),
                    self._datetime_format(new)
                ))
                snapshot_start = new
        return result

    def _sql_lookup(self, query, params):
        """
    ...
    def _generate_submission_steps(self, query, step_arguments, params):
        """
        ...
        if query in self.SNAPSHOT_TYPE_QUERIES:
            name = name + f": Timerange {params['snapshot_start']} to {params['snapshot_end']}"
        steps = [
            {
                "Name": name,
                "HadoopJarStep": {
    ...
    def _generate_emr_steps(self, dag, query, params):
        ...
        step_arguments = [
            self._fetch_hive_cli_dir(),
            self._sql_lookup(query, params)
        ]
        ...
        watcher = EmrStepSensor(
            task_id=watcher_task_id,
                job_flow_id=cluster_key,
            step_id="{{ task_instance.xcom_pull(task_ids=" + f"'{adder_task_id}'" + ",key='return_value')[0] }}",
            trigger_rule='none_failed',
            aws_conn_id='aws_default',
            dag=dag
        )
